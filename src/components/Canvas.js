...
const Canvas = ({ image, hat, hatPosition, setHatPosition, width, height }) => {
    ...
    useEffect(() => {
        ...
        baseImg.onload = () => {
            ...
            hatImg.onload = () => {
                ...
                ctx.save();
                // Move to position
                ctx.translate(hatPosition.x + (hatImg.width * hatPosition.scale) / 2, hatPosition.y + (hatImg.height * hatPosition.scale) / 2);
                // Rotate
    ...
    return (_jsx("canvas", { ref: canvasRef, width: width, height: height, style: {
            width: width * 0.75,
            height: height * 0.75,
            cursor: dragging.current ? 'grabbing' : 'grab',
            background: '#222'
        }, onMouseDown: onCanvasMouseDown, onMouseUp: onCanvasMouseUp, onMouseLeave: onCanvasMouseUp, onMouseMove: onCanvasMouseMove }));
};
